	...
	private final void processCreateNode(CreateNodeEvent createEvent) {
		...
		if ( relPath.startsWith( m_rootPath)) {
			...
			if ( m_filesysCtx.hasChangeHandler()) {
				...
				if ( m_filesysCtx.getChangeHandler().getGlobalNotifyMask() != null) {
					
					// Send a file created event to the change notification handler
					
					if ( createEvent.getFileType() == FileFolderServiceType.FILE)
					    m_filesysCtx.getChangeHandler().notifyFileChanged(NotifyAction.Added, relPath);
					else
					    m_filesysCtx.getChangeHandler().notifyDirectoryChanged(NotifyAction.Added, relPath);
		...
		if ( m_filesysCtx.hasChangeHandler()) {
			...
			if ( m_filesysCtx.getChangeHandler().getGlobalNotifyMask() != null) {
				...
				if ( deleteEvent.getFileType() == FileFolderServiceType.FILE)
				    m_filesysCtx.getChangeHandler().notifyFileChanged(NotifyAction.Removed, relPath);
				else
				    m_filesysCtx.getChangeHandler().notifyDirectoryChanged(NotifyAction.Removed, relPath);
