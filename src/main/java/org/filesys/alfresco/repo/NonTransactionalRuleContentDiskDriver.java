...
import org.filesys.alfresco.repo.rules.RuleEvaluator;
...
public class NonTransactionalRuleContentDiskDriver implements ExtendedDiskInterface 
{
    ...
    {
        FileStatus fileExists = diskInterface.fileExists(sess, tree, path);
            ...
            FileOpenParams params) throws IOException
    {
    	try
    	{
            ...
            if(logger.isDebugEnabled())
            {
                SharingMode sharedAccess = params.getSharedAccess();
                ...
                logger.debug("createFile:" + params.getPath() 
                    ...
                    + ", sharedAccess: " + sharedAccess.name()
            ...
            FileOpenParams param) throws IOException
    {
        ...
        if(logger.isDebugEnabled())
        {
            SharingMode sharedAccess = param.getSharedAccess();
